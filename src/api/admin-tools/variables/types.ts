...
export interface IVariablesApi {
  getCommonVariables(): Promise<Variable[]>;
  ...
  exportVariables(keys: string[], asArchive?: boolean): Promise<boolean>;
  importVariables(formData: FormData, isPreview: boolean): Promise<any>;
  getSecuredVariables(): Promise<SecretWithVariables[]>;
  getSecuredVariablesForSecret(secretName: string): Promise<Variable[]>;
  ...
  deleteSecuredVariables(secretName: string, keys: string[]): Promise<boolean>;
  createSecret(secretName: string): Promise<boolean>;
  downloadHelmChart(secretName: string): Promise<void>;
} 