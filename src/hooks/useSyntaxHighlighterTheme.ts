  ...
  const customTheme = useMemo(() => {
    ...
    if (typeof document !== 'undefined') {
      ...
      if (dataTheme) {
        ...
        const bgColor = getComputedStyle(document.documentElement).getPropertyValue('--vscode-editor-background').trim();
        if (bgColor) {
    ...
    const background = cssVar('--vscode-editor-background') || (darkTheme ? '#1f1f1f' : '#ffffff');
    const foreground = cssVar('--vscode-editor-foreground') || (darkTheme ? 'rgba(255, 255, 255, 0.85)' : 'rgba(0, 0, 0, 0.88)');
    ...
    const cssVarOrFallback = (varName: string, monacoClass: string, fallbackDark: string, fallbackLight: string): string => {
    ...
    const keywordColor = cssVarOrFallback('--vscode-symbolIcon-keywordForeground', 'mtk3', '#569CD6', '#0000FF');
    const stringColor = cssVarOrFallback('--vscode-symbolIcon-stringForeground', 'mtk9', '#CE9178', '#A31515');
    const numberColor = cssVarOrFallback('--vscode-symbolIcon-numberForeground', 'mtk10', '#B5CEA8', '#098658');
    const variableColor = cssVarOrFallback('--vscode-symbolIcon-variableForeground', 'mtk5', '#9CDCFE', '#001080');
    const functionColor = cssVarOrFallback('--vscode-symbolIcon-functionForeground', 'mtk12', '#DCDCAA', '#795E26');
    const typeColor = cssVarOrFallback('--vscode-symbolIcon-classForeground', 'mtk8', '#4EC9B0', '#267F99');
    const commentColor = cssVarOrFallback('--vscode-editorIndentGuide-activeBackground', 'mtk4', '#6A9955', '#008000');
    ...
    console.log('[useSyntaxHighlighterTheme] Token colors:', {
      keywordColor, stringColor, numberColor, variableColor, typeColor, commentColor
    });
