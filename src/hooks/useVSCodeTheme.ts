...
export const useVSCodeTheme = () => {
  ...
  const applyThemeToDocument = useCallback((theme: ThemeData) => {
    ...
    const isDarkTheme = theme.isDark ?? (theme.kind === 2 || theme.kind === 3);
    const isHighContrast = theme.isHighContrast ?? (theme.kind === 3);
    const themeClass = isHighContrast ? 'high-contrast' : (isDarkTheme ? 'dark' : 'light');
    ...
    root.style.setProperty('--vscode-is-high-contrast', isHighContrast ? '1' : '0');
    ...
    if (!isVSCodeWebview && theme.fonts) {
      ...
      root.style.setProperty('--vscode-font-size', `${theme.fonts.fontSize}px`);
      root.style.setProperty('--vscode-font-weight', theme.fonts.fontWeight);
      root.style.setProperty('--vscode-line-height', theme.fonts.lineHeight.toString());
    }

    // Set UI variables
    if (theme.ui) {
      root.style.setProperty('--vscode-tab-size', theme.ui.tabSize.toString());
      root.style.setProperty('--vscode-word-wrap', theme.ui.wordWrap);
    }

    // Set accessibility variables
    if (theme.accessibility) {
      root.style.setProperty('--vscode-high-contrast', theme.accessibility.highContrast ? '1' : '0');
      root.style.setProperty('--vscode-reduced-motion', theme.accessibility.reducedMotion);
    }

    // Apply VSCode colors as CSS variables
    if (!isVSCodeWebview) {
      if (theme.colors && Object.keys(theme.colors).length > 0) {
        console.log('QIP UI: Applying fallback colors for browser mode:', Object.keys(theme.colors).length);
        ...
        console.log('QIP UI: No VSCode colors received for browser mode, using stylesheet fallbacks');
      }
    }
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      const message = event.data as { type?: string; payload?: ThemeData; command?: string; theme?: ThemeData };

      // Debug: log theme messages
      if (message.type === 'theme-update' || message.command === 'themeUpdate') {
      ...
      if (message.type === 'theme-update' && message.payload) {
        ...
        const isDarkTheme = themeData.isDark ?? (themeData.kind === 2 || themeData.kind === 3);
        console.log('[useVSCodeTheme] Setting isDark:', isDarkTheme, 'from theme:', themeData.themeName);
