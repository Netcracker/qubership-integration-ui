...
const SecuredVariables: React.FC = () => {
  ...
  const refreshSecretVariables = useCallback(async (secret: string): Promise<boolean> => {
    ...
    if (response.success && response.data) {
      ...
      notificationService.requestFailed(response.error?.responseBody?.errorMessage || "Failed to refresh list.", null);
      return false;
    }
  ...
  const loadSecrets = useCallback(async () => {
    ...
    if (response.success && response.data) {
      ...
      notificationService.requestFailed(response.error?.responseBody?.errorMessage || "Failed to load secrets.", null);
    }
  ...
  const handleCreateSecret = useCallback(async (values: any) => {
    ...
    if (response.success) {
      ...
      notificationService.requestFailed(response.error?.responseBody?.errorMessage || "Failed to create secret.", null);
    }
  ...
  const handleAddVariable = useCallback(async (secret: string, key: string, value: string) => {
    ...
    if (response.success) {
      ...
      notificationService.requestFailed(response.error?.responseBody?.errorMessage || "Failed to add variable.", null);
    }
  ...
  const handleUpdateVariable = useCallback(async (secret: string, key: string, value: string) => {
    ...
    if (response.success) {
      ...
      notificationService.requestFailed(response.error?.responseBody?.errorMessage || "Failed to update variable.", null);
    }
  ...
  const handleDeleteVariable = useCallback(async (secret: string, key: string) => {
    const response = await variablesApi.deleteSecuredVariables(secret, [key]);
     if (response.success) {
       ...
       notificationService.requestFailed(response.error?.responseBody?.errorMessage || "Failed to delete variable.", null);
     }
  ...
  ), [variables, selectedKeys, newVariableKeys, editing, editingValue, calculateSecuredScrollHeight, handleUpdateVariable, handleDeleteVariable, handleAddVariable, setSelectedKeys]);

  return (
    <div className={commonStyles["container"]}>
      <div className={commonStyles["header"]}>
        <Title level={4} className={commonStyles["title"]}>
          <LockOutlined /> Secured Variables
        </Title>
