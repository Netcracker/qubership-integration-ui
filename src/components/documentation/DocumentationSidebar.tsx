...
const convertTOCToTreeData = (
  ...
  const key = hasDocument
    ? toDocRoutePath(routeBase, paths[node.documentId as number].replace(/\.md$/, ''))
...
const DocumentationSidebarComponent: React.FC<DocumentationSidebarProps> = ({ onSelect, collapsed = false }) => {
  const [treeData, setTreeData] = useState<DataNode[]>([]);
  const [selectedKeys, setSelectedKeys] = useState<React.Key[]>([]);
  const [expandedKeys, setExpandedKeys] = useState<React.Key[]>([]);
  const [expandableKeySet, setExpandableKeySet] = useState<Set<string>>(new Set());
  const [parentKeyByKey, setParentKeyByKey] = useState<Record<string, string | null>>({});
  ...
  useEffect(() => {
    Promise.all([loadTOC(), loadPaths()])
      .then(([toc, paths]) => {
        ...
        const initialExpanded = computeExpandedPath(currentPath, parentByKey, expandable);
        setExpandedKeys(initialExpanded);
        try {
          localStorage.setItem(EXPANDED_KEYS_STORAGE_KEY, JSON.stringify(initialExpanded));
        } catch (error) {
  ...
  useEffect(() => {
    ...
    if (currentPath && Object.keys(parentKeyByKey).length > 0) {
      const pathToExpand = computeExpandedPath(currentPath, parentKeyByKey, expandableKeySet);
      setExpandedKeys(pathToExpand);
      try {
        localStorage.setItem(EXPANDED_KEYS_STORAGE_KEY, JSON.stringify(pathToExpand));
      } catch (error) {
        console.error('Failed to save expanded keys to localStorage:', error);
      }
    }
  ...
  const toggleExpanded = useCallback((key: string) => {
    setExpandedKeys((prev) => {
      ...
      try {
        localStorage.setItem(EXPANDED_KEYS_STORAGE_KEY, JSON.stringify(newKeys));
      } catch (error) {
        console.error('Failed to save expanded keys to localStorage:', error);
      }
      return newKeys;
