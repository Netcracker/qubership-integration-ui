...
describe('documentationUrlUtils', () => {
  ...
  test('joinUrl joins base and path safely', () => {
    ...
    expect(joinUrl('https://example.com/docs/', 'toc.json')).toBe('https://example.com/docs/toc.json');
  });

  test('toDocMarkdownAssetPath builds docs path and extension', () => {
    expect(toDocMarkdownAssetPath('01__Chains/chains')).toBe('docs/01__Chains/chains.md');
    expect(toDocMarkdownAssetPath('docs/01__Chains/chains.md')).toBe('docs/01__Chains/chains.md');
