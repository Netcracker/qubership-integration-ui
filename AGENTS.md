...
The application targets two environments: browser and VSCode webview.
The application supports four theme modes: light, dark, high contrast, and VSCode webview mode.
...
```
src/
├── api/            — Api interface, REST/VSCode implementations, all business types
...
| **VSCode Webview** | `:root.vscode-webview` class | Real `--vscode-*` variables injected by the extension host |

In VSCode webview mode the extension host injects the actual IDE theme colors as CSS variables,
...
Detect current theme in components via `useVSCodeTheme()` hook:
    ...
    - `isVSCodeWebview` — use to adapt behavior when running inside the VSCode extension.
...
Integration chain elements' parameters forms are generated from JSON schemas using `@rjsf/antd`. These schemas dynamically loaded at runtime:
```typescript
...
2. Element icons (custom)
3. Runtime overrides (from config)
...
SVG strings are normalized: hard-coded colors → `currentColor` for theme support.
...
src/api/rest/vscodeExtensionApi.ts    — VSCode messaging implementation
