...
function main() {
    ...
    getSortedMarkdownDocuments(docDir)
        .then(paths =>
            Promise.all([
                buildPaths(docDir, paths).then(paths => saveData(outDir, 'paths.json', paths, 'document paths')),
                buildNames(docDir, paths).then(names => saveData(outDir, 'names.json', names, 'document names')),
                buildTocTree(docDir, paths).then(tree => saveData(outDir, 'toc.json', tree, 'table of content tree')),
