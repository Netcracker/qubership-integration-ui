...
describe('getCumulativeStatus', () => {
  ...
  it('returns highest priority status for multiple statuses', () => {
    ...
    expect(getCumulativeStatus(new Set(['DEPLOYED', 'PROCESSING']))).toBe('PROCESSING');
    expect(getCumulativeStatus(new Set(['FAILED', 'PROCESSING']))).toBe('PROCESSING');
    expect(getCumulativeStatus(new Set(['DRAFT', 'DEPLOYED']))).toBe('DEPLOYED');
    expect(getCumulativeStatus(new Set(['DRAFT', 'FAILED', 'DEPLOYED']))).toBe('FAILED');
  });

  it('ignores unknown status values', () => {
    // Unknown values shouldn't break the function
    expect(getCumulativeStatus(new Set(['UNKNOWN', 'DEPLOYED']))).toBe('DEPLOYED');
